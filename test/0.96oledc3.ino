...
#define LED_COUNT 1
#define LED_PIN 8         // GPIO8 cho WS2812
#define BUILTINLED 8      // GPIO8 cho LED tích hợp (kiểm tra bo mạch)
#define BUTTON_PIN 3      // GPIO3 cho nút nhấn
#define SCREEN_WIDTH 128
#define SCREEN_HEIGHT 64
#define OLED_SDA 6        // GPIO6 cho SDA
#define OLED_SCL 7        // GPIO7 cho SCL
...
// Hàm đọc điện áp pin (điều chỉnh cho ESP32-C3)
float readBatteryVoltage() {
    const float alpha = 0.1;
    const float R1 = 100000;
    const float R2 = 10000;
    const float ADC_REF_VOLTAGE = 3.3; // ESP32-C3 dùng 3.3V
    static float emaValue = 0;
    int rawADC = analogRead(0); // GPIO0 cho ADC trên ESP32-C3
...
// Hàm cài đặt (điều chỉnh cho ESP32-C3)
void setup() {
    Serial.begin(115200);
    pinMode(BUTTON_PIN, INPUT_PULLUP);
    pinMode(BUILTINLED, OUTPUT);
    Wire.begin(OLED_SDA, OLED_SCL);
    delay(50); // Đợi I2C ổn định
