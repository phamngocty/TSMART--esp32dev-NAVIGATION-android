...
#define TFT_RST       3   // GPIO3 - Reset
#define TFT_SCLK      6   // GPIO6 - SPI Clock (mặc định trên ESP32-C3)
#define TFT_MOSI      7   // GPIO7 - SPI MOSI (mặc định trên ESP32-C3)
#define BUTTON_PIN    8   // GPIO8 - Nút bấm (tránh GPIO0 vì chức năng boot mode)
#define BUILTINLED    9   // GPIO9 - LED tích hợp (kiểm tra module có hỗ trợ không)
#define LED_PIN       4   // GPIO4 - LED WS2812 (đổi từ GPIO3 để tránh xung đột)
...
void displayTime(bool smallFont) {
    ...
    if (!slideComplete) {
        ...
        if (currentTimeMillis - lastSlideUpdate >= slideInterval) {
            ...
            myTFT.setCursor(centerX - 25, 10 + slideOffsetY);
            myTFT.print("TIME");
...
void drawArcProgress(int16_t centerX, int16_t centerY, int16_t outerRadius, int16_t innerRadius, int16_t startAngle, int16_t endAngle, float progress) {
    static float prevProgress = -1.0; // Lưu giá trị progress trước đó
...
void displayNavigation() {
    ...
    if (slideComplete) {
        ...
        if (app.navDirection != prevDirection) {
            ...
            myTFT.fillRect(0, 60, 240, 20, 0x0000); // Xóa vùng cũ
            myfont.print(20, 60, direction, 0xC618, 0x0000); // Xám nhạt
        ...
        if (app.navETA != prevETA) {
            myfont.set_font(vietnamtimes12);
            myfont.print(-60, 135, "          ", 0x0000, 0x0000); // Xóa vùng cũ
            myfont.print(-60, 135, app.navETA, 0xFFFF, 0x0000); // Trắng
            prevETA = app.navETA;
        }
        ...
        if (app.navDistance != prevDistance) {
            myfont.set_font(vietnamtimes12);
            myTFT.fillRect(175, 135, 50, 20, 0x0000); // Xóa vùng cũ
            myfont.print(175, 135, app.navDistance, 0xFFFF, 0x0000); // Trắng
            prevDistance = app.navDistance;
        }

        // Vẽ mũi tên
        if (watch.getNavigation().hasIcon && watch.getNavigation().active) {
            myTFT.drawBitmap(centerX - 24, 100, watch.getNavigation().icon, 48, 48, 0x06FE, 0x0000); // Mũi tên xanh
        }
...
float readBatteryVoltage() {
    ...
    int rawADC = analogRead(0); // GPIO0 (ADC1_CH0, kiểm tra module không dùng GPIO0 cho chức năng khác)
...
void setup() {
    ...
    SPI.setFrequency(40000000); // 40 MHz để đảm bảo ổn định
    SPI.setDataMode(SPI_MODE0);
    SPI.setBitOrder(MSBFIRST);

    // Khởi tạo TFT
    myTFT.begin(40000000);
    myTFT.setRotation(0);
    myTFT.fillScreen(COLOR_BG);
